...
namespace Stomatologia.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            //wyszukuje użytkownika po podanym loginie
            var user = await UserManager.FindByNameAsync(model.Email);
            //sprawdza czy hasło jest poprawne
            var check = await UserManager.CheckPasswordAsync(user, model.Password);
            //sprawdza czy istnieje użytkoninik z danym hasłem i loginem i czy ma maila niepotwierdzonego
            if (check == true  && !user.EmailConfirmed)
            {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    UserManager.AddToRole(user.Id, "Pacjent");
                    ...
                    //lilnk do strony wygenerowany w mailu
        ...
        public ActionResult ShowUsers(int? error)
        {
            if (error.HasValue)
                ViewBag.Error = "Nie można usunąć admina";
        ...
        [Authorize(Roles = "Admins")]
        public ActionResult DodajLekarza()
        {
            return View();
        }
        [HttpPost]
        [Authorize(Roles = "Admins")]
        public async Task<ActionResult> DodajLekarza(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email };
                ...
                if (result.Succeeded)
                {
                    ...
                    await UserManager.SendEmailAsync(
                                ...
                                "Twoje hasło to:"+ model.Password +". Potwierdź swoją rejestracje klikając na podany link: " +
                                 "<a href=\"" + callbackUrl + "\">Potwierdź</a>");
                    // For more information on how to enable account confirmation and password reset please visit http://go.microsoft.com/fwlink/?LinkID=320771
                    // Send an email with this link
                    // string code = await UserManager.GenerateEmailConfirmationTokenAsync(user.Id);
        ...
        {
            if (UserManager.IsInRole(id, "Admins"))
            {               
              
                if (Request.IsAjaxRequest())
                {
                    ViewBag.Error = "Nie można usunąc admina";
                    return PartialView("_DeleteError");
                }

                return RedirectToAction("ShowUsers", new { error = 1 });
            }
